...
const gender = yup
  .string()
  .notOneOf(["0"], "Selecione um sexo")
  .required(ERROR_REQUIRED);

const days = yup
  .string()
  .matches(PATTERN_NUMBER, ERROR_INVALID_PATTERN)
  .required(ERROR_REQUIRED);

const description = yup.string().trim().required(ERROR_REQUIRED);
...
export const roadMapFormSchema = yup.object().shape({
  roadName: name,
  days,
  description: yup.string().trim(),
});

export const postsFormSchema = yup.object().shape({
  location: name,
  description
});
